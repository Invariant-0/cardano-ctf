...
import { createPurchaseOfferDatumSchema } from './types';
...
export async function setup(lucid: LucidEvolution): Promise<GameData> {
  ...
  const policyMapping = {
    ...
    POLICY1: (validators.nftPolicyId[0] !== 'f' ? 'f' : 'd') + validators.nftPolicyId.slice(1),
  };

  const decodeAssets = (prescription: Prescription): Prescription => ({
    ...prescription,
    desiredAssetPolicyId:
      policyMapping[prescription.desiredAssetPolicyId as keyof typeof policyMapping],
    desiredAssetTokenName:
      prescription.desiredAssetTokenName !== null
        ? tokenNameMapping[prescription.desiredAssetTokenName as keyof typeof tokenNameMapping]
        : null,
  });
  const offerPrescriptions = _.shuffle(
    JSON.parse(decodeBase64(encodedPrescriptions)) as Prescription[]
  ).map(decodeAssets);

  console.log('Minting a precious NFT and creating multiple purchase offers...');
  const tx = lucid
    .newTx()
    .collectFrom([nftOriginUtxo])
    .attach.MintingPolicy(validators.nftPolicy)
    .mintAssets({ [asset]: BigInt(1) }, Data.void());

  const enhancedTx = offerPrescriptions.reduce(
    (
      accTx,
      { owner, desiredAssetPolicyId, desiredAssetTokenName, setProtocolStakingCredential }
    ) => {
      ...
      return accTx.pay.ToContract(
        ...
        {
          kind: 'inline',
          value: createPurchaseOfferDatumSchema(owner, desiredAssetPolicyId, desiredAssetTokenName),
        },
  ...
  console.log(
    `NFT was minted and put to your balance. Purchase offers were also created${getFormattedTxDetails(
      txHash,
      lucid
    )}`
  );
  ...
  const scriptUtxos = filterUTXOsByTxHash(
    await lucid.utxosAt(
      scriptHashToCredential(validatorToScriptHash(validators.purchaseOfferValidator))
    ),
    txHash
  );
...
export async function test(
  ...
  if (balanceDiff > offerredPrice) {
    passTest(`TEST 1 PASSED - you earned more than the offerred price`, lucid);
  } else {
