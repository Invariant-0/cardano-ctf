...
export async function setup(lucid: LucidEvolution) {
  ...
  // Create player's account datum
  const playerAccountDatum = {
    balance: playerDepositAmount,
    owner: playerPkh,
  };
  ...
  const bankBalanceAfterVictim = initialBankUtxo.assets.lovelace + victimDepositAmount;

  const victimAccountTx = await lucid
    .newTx()
    ...
    .pay.ToContract(
      bankAddress,
      { kind: 'inline', value: Data.void() },
      { lovelace: bankBalanceAfterVictim }
    )
  ...
  const victimAccountSignedTx = await victimAccountTx.sign.withWallet().complete();
  const victimAccountTxHash = await victimAccountSignedTx.submit();
  await awaitTxConfirms(lucid, victimAccountTxHash);

  // Get the created UTxOs
  const bankUtxoAfterVictim = filterUTXOsByTxHash(
    await lucid.utxosAt(bankAddress),
    victimAccountTxHash
  )[0];
  ...
  // Third transaction: Create player's account with minted account token
  console.log('Setting up the player account...');

  const finalBankBalance = bankUtxoAfterVictim.assets.lovelace + playerDepositAmount;

  const playerAccountTx = await lucid
    .newTx()
    // Spend the bank UTxO to satisfy account minting policy
    .collectFrom([bankUtxoAfterVictim], Data.void())
    .attach.SpendingValidator(bankValidator)
    // Reference the config
    .readFrom([configUtxo])
    // Mint single account validity token
    ...
    .pay.ToContract(
      bankAddress,
      { kind: 'inline', value: Data.void() },
      { lovelace: finalBankBalance }
    )
    // Create the player's account with validity token
    .pay.ToContract(
      accountAddress,
  ...
  const playerAccountSignedTx = await playerAccountTx.sign.withWallet().complete();
  const playerAccountTxHash = await playerAccountSignedTx.submit();
  await awaitTxConfirms(lucid, playerAccountTxHash);

  // Get the created UTxOs
  const bankUtxo = filterUTXOsByTxHash(await lucid.utxosAt(bankAddress), playerAccountTxHash)[0];
  const playerAccountUtxo = filterUTXOsByTxHash(
    await lucid.utxosAt(accountAddress),
    playerAccountTxHash
  )[0];

  console.log(
    `Bank initialized with victim's ${victimDepositAmount / 1_000_000n} ADA and player's ${playerDepositAmount / 1_000_000n} ADA deposits`
  );
  ...
  console.log(`Can you drain the bank by exploiting the missing security check?\n`);
...
export async function test(
  ...
  if (passed) {
    await submitSolutionRecord(lucid, 110n); // bank_10 level

    passAllTests(
      '\nCongratulations on completing Bank 10: Transfer!\n' +
        'You successfully exploited the missing security check in the transfer feature!\n' +
        'Remember: During refactors and feature additions, it is crucial to ensure all past security assumptions are still enforced!\n' +
