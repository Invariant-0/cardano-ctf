...
- **Account Validity Tokens**: Since validators cannot run on UTxO creation, the bank now requires
  ...
  tokens must be minted when accounts are created (requiring bank validation so initial deposits are
  checked) and must be present in all account UTxOs.
...
configuration can not be modified. However, equipped with our thinking from the previous level and
closely inspecting the contracts, there surely is a way.
