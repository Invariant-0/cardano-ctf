...
import { createMultisigDatum, createTreasuryDatum } from './types';
...
export async function setup(lucid: LucidEvolution) {
  ...
  const createTreasuryTx = await lucid
    ...
    .pay.ToContract(
      ...
      { kind: 'inline', value: createTreasuryDatum(treasuryFunds, owners) },
  ...
  const createMultisigTx = await lucid
    .newTx()
    .pay.ToContract(
      validators.multisigAddress,
      {
        kind: 'inline',
        value: createMultisigDatum(multisigRelease, multisigBeneficiary, owners, [])!,
      },
  ...
  console.log(
    `Multisig setup transaction was submitted${getFormattedTxDetails(submittedMSTx, lucid)}`
  );
...
export async function test(
  ...
  if (endBalance - gameData.originalBalance < 5000000n) {
    failTest('TEST 1 FAILED -- you did not obtain enough additional ADA by unlocking the treasury');
    passed = false;
  } else {
