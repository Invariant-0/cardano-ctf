...
export async function setup(lucid: LucidEvolution) {
  ...
  console.log(
    `Can you still manage to get the account token and use it to drain the bank again? Remember the name of the task.\n`
  );
...
export async function test(
  ...
  if (passed) {
    await submitSolutionRecord(lucid, 108n); // bank_08 level

    passAllTests(
      '\nCongratulations on completing Bank 08: Self Collision!\n' +
        'You successfully found yet another, more obscure way to obtain the account token and used it to drain the bank, gj!\n' +
        'You see, it does not really matter how obscure the way is, as long as it works.\n' +
